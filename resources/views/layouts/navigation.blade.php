<nav x-data="{ open: false }" class="bg-gray-100 border-b border-gray-100">
    ...
    <div class="px-4">
        <div class="flex justify-between h-16">
            <div class="flex gap-10 overflow-hidden">
                ...
                <!-- Navigation Links -->
                <div class="hidden gap-8 sm:flex w-full overflow-auto">
                    @if (Auth::user() && Auth::user()->hasRole('Admin'))
                        <x-nav-link :href="route('dashboard.working-times.index')" :active="Str::startsWith(request()->route()->getName(), 'dashboard')">
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')">
                            {{ __('Изменить профиль пользователя') }}
                        </x-dropdown-link>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if (Auth::user() && Auth::user()->hasRole('Admin'))
                <x-responsive-nav-link :href="route('dashboard.working-times.index')" :active="Str::startsWith(request()->route()->getName(), 'dashboard')">
                    {{ __('Панель управления') }}
